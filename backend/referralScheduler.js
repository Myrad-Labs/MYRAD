...
cron.schedule("*/15 * * * * *", async () => {
  ...
  try {
    ...
    // ==============================================
    // 1️⃣ Generate referral codes (UNCHANGED)
    // ==============================================
    await query(`
    ...
    // ==============================================
    // 3️⃣ Reward referrers (Efficient Version)
    // ==============================================
    ...
    for (const ref of refs.rows) {
      ...
      // how many 50pt rewards already given?
...
if (expectedPoints > alreadyRewardedPoints) {
  ...
  console.log(`Gave ${difference} points to referrer ${referrerId}`);
      ...
      const alreadyRewarded = parseInt(given.rows[0].count);

      if (successCount > alreadyRewarded) {

        const difference = successCount - alreadyRewarded;
        const totalReward = difference * 50;

        // Insert ONE aggregated history entry
        await query(`
          INSERT INTO points_history (id, user_id, points, reason, created_at)
          VALUES (gen_random_uuid(), $1, $2, 'referral_success_bonus', NOW());
        `, [referrerId, totalReward]);
        ...
        await query(`
          ...
          SET total_points = total_points + $1
          WHERE id = $2;
        `, [totalReward, referrerId]);

        console.log(`Gave ${totalReward} points to referrer ${referrerId}`);
      }
    }
